...
import { useMedianet } from '../context/MedianetContext';

const RegistrationKycScreen = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const { completeMedianet } = useMedianet();

    // Retrieve isExistingUser from location.state
    const { isExistingUser = false, phoneNumber, firstName, lastName } = location.state || {};
    ...
    const handleSubmit = async (e) => {
        ...
        if (isValid && !isLoading) {
            try {
                ...
                if (isExistingUser) {
                    // For existing users, skip verification and go to OTP page
                    ...
                    // For new users, verify phone number
